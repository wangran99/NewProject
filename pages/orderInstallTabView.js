...
import OrderInstallListItem from '../component/orderInstallListItem'
...
type Props = {};
export default class orderInstallTabView extends Component<Props> {
    ...
    _getOrderInstallList() {
        httpApi.getOrderInstallList(this.pageIndex, this.status).then(data => {
            if (this.pageIndex == 1)
                this.setState({ data });
    ...
    _refreshOrderInstallList() {
        this.pageIndex = 1;
        this._getOrderInstallList();
    }
    componentDidMount() {
        this._getOrderInstallList();
    }
    ...
    render() {
        ...
        return (
            <FlatList
                ...
                onRefresh={this._refreshOrderInstallList.bind(this)}
                onEndReached={this._getOrderInstallList.bind(this)}
                ...
                ListEmptyComponent={() => {
                    return (
                        ...
                        <TouchableOpacity
                            style={{ flex: 1 }}
                            onPress={() => this._refreshOrderInstallList()}
                            activeOpacity={0.3}>
                ...
                renderItem={({ item }) => <OrderInstallListItem data={item} navigator={navig} style={{ marginTop: 5 }}
                    onPress={() => {
                        let a = 1;
                        navig.navigate("OrderInstallDetail", { id: item.id })
                    }} />}
...
//输出一个类
module.exports = orderInstallTabView;