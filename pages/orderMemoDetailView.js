...
import { TouchableOpacity, StyleSheet, Text, AsyncStorage, Image, View, DeviceEventEmitter } from 'react-native';
...
type Props = {};
export default class orderMemoDetailView extends Component<Props> {
    static thiz = this;
    static data;
        ...
        return {
            ...
            headerRight: (
                <TouchableOpacity onPress={() => {
                    let orderId = navigation.getParam('orderId', null);

                    httpApi.memoDetailByOrderid(orderId).then(data => {
                        if (JSON.stringify(data) == "{}") {
                            navigation.navigate("AddMemo", { from: 'order', oderId: orderId })
                        } else
                            navigation.navigate("EditMemo", { from: 'order', oderId: orderId })
                    });
                }}>
    ...
    componentDidMount() {
        ...
        this.orderId = navigation.getParam('orderId', null);

        this._getMemoDetailByOrderId();
        //收到监听
    ...
    _getMemoDetailByOrderId() {
        httpApi.memoDetailByOrderid(this.orderId).then(data => {
            if (JSON.stringify(data) != "{}") {
                this.setState({ data });
...
//输出一个类
module.exports = orderMemoDetailView;