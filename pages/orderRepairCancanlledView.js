...
type Props = {};
export default class orderRepairCancanlledView extends Component<Props> {
    ...
    _getOrderRepairList() {
        httpApi.getOrderRepairList(this.pageIndex, 2).then(data => {
            if (this.pageIndex == 1)
                this.setState({ data });
    ...
    render() {
        ...
        return (
            <FlatList
                ...
                renderItem={({ item }) => <OrderRepairListItem data={item} style={{ marginTop: 5 }}
                    onPress={() => {
                        // let a = 1;
...
//输出一个类
module.exports = orderRepairCancanlledView;